...
Elements = []
for name in ip:
    if name in ['Xbeam', 'simulation', 'meta']:
        continue
    obj = ip[name]
...
XFEL_photon_E = ip['Xbeam']['photonenergy']
params['photons_total']  = float(ip['Xbeam']['photons_total']) if 'photons_total' in ip['Xbeam'] else None
params['X_FWHM_duration'] = float(ip['Xbeam']['X_FWHM_duration']) if 'X_FWHM_duration' in ip['Xbeam'] else None
...
params.update({
    ...
    'fig_rows': 4,
    'fig_cols': 5,
    'beamsize': float(ip['Xbeam']['size']),
    'gauss_x_shift': float(ip['Xbeam']['offset']),
    'gauss_x_tilt': vibe.yamlval('tilt', ip['Xbeam'], 0),
    'remove_ticks': 1
})
